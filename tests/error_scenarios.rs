...
mod error_scenarios {
    use rs_vio::estimator::{
        ResilientAsyncFeatureDetector, ResilientAsyncOptimizer, PipelineError,
        PipelineMetrics, RecoveryStrategy, RecoveryContext,
    };
    use rs_vio::datasets::config::{FeatureDetectionConfig, Config};
    ...
    async fn test_feature_detection_timeout_with_large_image() {
        ...
        let detector = ResilientAsyncFeatureDetector::<3>::new(&config, None)
            .with_timeout(1);  // 1ms timeout - very tight
    ...
    async fn test_timeout_doesnt_block_subsequent_frames() {
        ...
        let _result1 = detector.detect_features(left.clone(), right.clone(), &mut frame1).await;
    ...
    async fn test_metrics_track_errors() {
        ...
        for i in 0..3 {
            ...
            let _ = detector.detect_features(left.clone(), right.clone(), &mut frame).await;
        }
    ...
    fn test_exponential_backoff() {
        let strategy = RecoveryStrategy::Retry { max_attempts: 5, backoff_ms: 10 };
    ...
    fn test_error_rate_calculation() {
        ...
        assert!(rate >= 0.0 && rate <= 1.0, "Error rate should be valid: {}", rate);
    }
    ...
    fn test_optimizer_timeout_configuration() {
        ...
        let _optimizer = ResilientAsyncOptimizer::new(&config, None)
            .with_timeout(750);
    ...
    async fn test_end_to_end_error_recovery() {
        ...
        let detector = ResilientAsyncFeatureDetector::<3>::new(&detector_config, Some(metrics.clone()));
