...
use rs_vio::estimator::{
    CircuitBreaker, CircuitBreakerConfig, CircuitState, HardwareProfile, MetricsServer,
    MetricsServerConfig, OtelMetricsBatch, PipelineMetrics, PrometheusMetrics, SwarmHealthStatus,
    TimeoutAutoTuner,
};
...
fn test_hardware_profile_selection() {
    ...
    for (name, profile) in profiles {
        ...
        assert!(
            profile.detection_timeout_ms() > 0,
            "Profile {} has no detection timeout",
            name
        );
        // Optimization timeout should be at least equal to detection timeout
        assert!(
            profile.optimization_timeout_ms() >= profile.detection_timeout_ms(),
            "Profile {} has opt={} < det={}",
            name,
...
fn test_hardware_adaptive_timeout_stack() {
    ...
    for profile in profiles {
        ...
        for _ in 0..100 {
            tuner.observe_detection(profile.base_detection_us * 2);
        }
