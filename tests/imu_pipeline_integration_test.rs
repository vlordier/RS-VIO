...
fn test_complete_imu_pipeline_stationary() {
    ...
    assert!(
        decomp.accel_bias.norm() < 0.5,
        "Accel bias should be small: {}",
        decomp.accel_bias.norm()
    );
    assert!(
        decomp.gyro_bias.norm() < 0.1,
        "Gyro bias should be small: {}",
        decomp.gyro_bias.norm()
    );

    // Signal quality should be high (low noise)
    assert!(
        decomp.quality.snr[0] > 10.0,
        "SNR should be reasonable for clean signal: {}",
        decomp.quality.snr[0]
    );

    // Fundamental frequency should be zero (no harmonics)
    assert_eq!(
        decomp.quality.fundamental_freq_hz, 0.0,
...
fn test_complete_imu_pipeline_flight_with_vibration() {
    ...
    assert!(
        decomp.quality.rms[0] > 0.5,
        "RMS should be significant with vibration: {}",
        decomp.quality.rms[0]
    );
    assert!(
        decomp.quality.rms[1] > 0.5,
        "RMS should be significant with vibration: {}",
    ...
    if decomp.quality.fundamental_freq_hz > 0.0 {
        assert!(
            decomp.quality.fundamental_freq_hz >= 10.0
                && decomp.quality.fundamental_freq_hz <= 1000.0,
            "Frequency should be in valid range: {}",
...
fn test_pipeline_motor_state_transitions() {
    ...
    assert!(
        matches!(state_2, MotorState::Running | MotorState::Transitioning),
    ...
    assert!(
        matches!(state_4, MotorState::Off | MotorState::Transitioning),
...
fn test_pipeline_tilted_orientation_gravity_recovery() {
    ...
    let gravity_mag = decomp.gravity.norm();
    assert!(
        (gravity_mag - 9.81).abs() < 0.3,
        "Gravity magnitude should be preserved: expected 9.81, got {}",
        gravity_mag
    );
    ...
    assert!(
        (decomp.gravity.y - expected_y).abs() < 0.5,
        "Y component should be ~{}, got {}",
        expected_y,
        decomp.gravity.y
    );
    assert!(
        (decomp.gravity.z - expected_z).abs() < 0.5,
        "Z component should be ~{}, got {}",
        expected_z,
        decomp.gravity.z
    );
}
...
fn test_pipeline_bias_convergence_with_drift() {
    ...
    assert!(
        decomp.accel_bias.norm() < 5.0,
        "Bias estimate should be bounded: {}",
        decomp.accel_bias.norm()
    );
    assert!(
        decomp.accel_bias.norm().is_finite(),
...
fn test_pipeline_multi_frequency_vibration() {
    ...
    let total_rms = (decomp.quality.rms[0].powi(2)
        ...
        + decomp.quality.rms[2].powi(2))
    .sqrt();

    assert!(
        total_rms > 0.5,
        "Combined RMS should show significant vibration: {}",
        total_rms
    );
}
...
fn test_pipeline_noise_floor_adaptation() {
    ...
    assert!(
        snr_off > 30.0,
        "SNR should be high when motors off: {}",
        snr_off
    );
    ...
    assert!(
        snr_on.is_finite() && snr_on > 0.0,
        "SNR should be valid when motors on: {}",
        snr_on
    );
