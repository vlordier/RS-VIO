...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Process frames
    println!("Processing {} frames...", frames_to_process.min(sequence.num_frames()));
    ...
    println!("\n✓ VIO processing complete in {:.2}s", process_time.as_secs_f64());
    ...
    println!("    ./datasets/tum_vi/{}/mav0/mocap0/data.csv", sequence_name);
    
    Ok(())
}

/// Save trajectory in TUM format: timestamp tx ty tz qx qy qz qw
fn save_trajectory_tum(filename: &str, trajectory: &[(i64, na::Matrix4<f64>)]) -> Result<(), Box<dyn std::error::Error>> {
