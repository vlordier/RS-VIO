...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let frames_to_process = args.get(2)
    ...
    println!("✓ Loaded sequence in {:.2}s", load_start.elapsed().as_secs_f64());
    ...
    for i in 0..frames_to_process.min(sequence.num_frames()) {
        ...
        if (i + 1) % 100 == 0 {
            println!("  Processed {}/{} frames...", i + 1, frames_to_process.min(sequence.num_frames()));
        }
    }
    
    let process_time = process_start.elapsed();
    
    println!("\n✓ Phase 8B processing complete in {:.2}s", process_time.as_secs_f64());
    println!("  - Average: {:.2}ms per frame", process_time.as_secs_f64() * 1000.0 / processed_count as f64);
    println!("  - IMU samples processed: {}", total_imu_samples);
    println!("  - IMU samples per frame: {:.1}", total_imu_samples as f64 / processed_count as f64);
    
    // Extract trajectory
    let trajectory = estimator.get_trajectory();
    println!("  - Estimated {} poses (with velocity states)", trajectory.len());
...
fn save_trajectory(filename: &str, trajectory: &[(i64, na::Matrix4<f64>)]) -> Result<(), Box<dyn std::error::Error>> {
    use std::fs::File;
    use std::io::Write;
