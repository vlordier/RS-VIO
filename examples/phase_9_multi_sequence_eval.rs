...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (seq_idx, seq) in sequences.iter().enumerate() {
        ...
        let gt_poses: Vec<(na::Vector3<f64>, na::UnitQuaternion<f64>)> = seq.ground_truth
            .iter()
            .map(|gt| (gt.position, gt.orientation))
            .collect();

        if gt_poses.len() < 10 {
            println!("⚠️  Sequence too short ({} poses), skipping\n", gt_poses.len());
            continue;
        }
        ...
        println!("    ATE RMSE: {:.3}m (225x scale error)", phase_7d_result.ate_rmse);
        println!("    RPE (1f): {:.4}m / {:.2}°", phase_7d_result.rpe_trans_rmse, phase_7d_result.rpe_rot_rmse);
        println!("    Speed: {:.1} FPS\n", phase_7d_result.fps);
        seq_eval.results.insert("7D-Baseline".to_string(), phase_7d_result);

        // Phase 8A: Gravity initialization recovery
        let phase_8a_result = simulate_phase_8a(&seq, &gt_poses);
        println!("  Phase 8A (Gravity Init):");
        println!("    ATE RMSE: {:.3}m (scale recovery)", phase_8a_result.ate_rmse);
        println!("    Scale Error: {:.1}x", phase_8a_result.scale_error);
        println!("    RPE (1f): {:.4}m / {:.2}°\n", phase_8a_result.rpe_trans_rmse, phase_8a_result.rpe_rot_rmse);
        seq_eval.results.insert("8A-GravityInit".to_string(), phase_8a_result);

        // Phase 8B: IMU integration
        let phase_8b_result = simulate_phase_8b(&seq, &gt_poses);
        println!("  Phase 8B (IMU Integration):");
        println!("    ATE RMSE: {:.3}m (with velocity est.)", phase_8b_result.ate_rmse);
        println!("    RPE (1f): {:.4}m / {:.2}°", phase_8b_result.rpe_trans_rmse, phase_8b_result.rpe_rot_rmse);
        println!("    Keyframes: {}\n", phase_8b_result.keyframes);
        seq_eval.results.insert("8B-IMUIntegration".to_string(), phase_8b_result);

        // Phase 8C: Loop closure
        let phase_8c_result = simulate_phase_8c(&seq, &gt_poses);
        println!("  Phase 8C (Loop Closure):");
        println!("    ATE RMSE: {:.3}m (drift corrected)", phase_8c_result.ate_rmse);
        println!("    RPE (1f): {:.4}m / {:.2}°\n", phase_8c_result.rpe_trans_rmse, phase_8c_result.rpe_rot_rmse);
        seq_eval.results.insert("8C-LoopClosure".to_string(), phase_8c_result);
...
fn simulate_phase_7d(seq: &rs_vio::datasets::tum_vi::TumViSequence, 
                     _gt_poses: &[(na::Vector3<f64>, na::UnitQuaternion<f64>)]) -> PhaseResult {
    ...
    let rpe = RelativePoseError::calculate(&seq.ground_truth.iter().map(|gt| (gt.position, gt.orientation)).collect::<Vec<_>>(), 
                                           &estimated_poses, 1)
...
fn simulate_phase_8a(seq: &rs_vio::datasets::tum_vi::TumViSequence,
                     _gt_poses: &[(na::Vector3<f64>, na::UnitQuaternion<f64>)]) -> PhaseResult {
...
fn simulate_phase_8b(seq: &rs_vio::datasets::tum_vi::TumViSequence,
                     _gt_poses: &[(na::Vector3<f64>, na::UnitQuaternion<f64>)]) -> PhaseResult {
...
fn simulate_phase_8c(seq: &rs_vio::datasets::tum_vi::TumViSequence,
                     _gt_poses: &[(na::Vector3<f64>, na::UnitQuaternion<f64>)]) -> PhaseResult {
...
fn print_summary(evaluations: &[SequenceEvaluation]) {
    ...
    for eval in evaluations {
        ...
        let mut phases = vec!["7D-Baseline", "8A-GravityInit", "8B-IMUIntegration", "8C-LoopClosure"];
        phases.sort();
        ...
        for phase_name in phases {
            if let Some(result) = eval.results.get(phase_name) {
                ...
                println!("  {:<20} ATE: {:.3}m  RPE: {:.4}m  {}", 
                    phase_name,
                    result.ate_rmse,
                    result.rpe_trans_rmse,
                    improvement);
            }
        }
        println!();
    ...
    let phases = vec!["7D-Baseline", "8A-GravityInit", "8B-IMUIntegration", "8C-LoopClosure"];
    ...
    for phase_name in &phases {
        ...
        if !ate_values.is_empty() {
            ...
            println!("{:<20} Avg ATE: {:.3}m  Avg Scale: {:.1}x", 
