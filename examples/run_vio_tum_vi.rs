...
use rs_vio::types::{UnitQuaternion, Vector3};
use std::env;
use std::fs::File;
use std::io::Write;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let duration_secs = if !sequence.cam0_timestamps.is_empty() {
        (sequence.cam0_timestamps.last().unwrap() - sequence.cam0_timestamps.first().unwrap())
            as f64
            / 1e9
    } else {
        0.0
    ...
    println!(
        ...
        vio_duration.as_secs_f64()
    );
    println!(
        "  - Average: {:.2}ms per frame",
        vio_duration.as_secs_f64() * 1000.0 / frame_count as f64
    );
    ...
    if !estimated_trajectory.is_empty() {
        let duration = (estimated_trajectory.last().unwrap().timestamp_ns
            - estimated_trajectory.first().unwrap().timestamp_ns) as f64
            / 1e9;
        ...
        println!(
            "  - Rate: {:.1} Hz",
            estimated_trajectory.len() as f64 / duration
        );
    }
    ...
    println!(
        "   python scripts/evaluate_trajectory.py {} datasets/tum_vi/{}/mav0/mocap0/data.csv",
