...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let num_frames: usize = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(1000);
    ...
    for i in 0..frames_to_process {
        ...
        let _imu_window: Vec<_> = sequence
            .imu_data
            .iter()
            .filter(|imu| {
                if i > 0 {
    ...
    if realtime_factor >= 1.0 {
        ...
        println!(
            "  ⚠ Below real-time ({:.0}% of required speed)",
