...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: {} <estimated_trajectory.txt> <ground_truth.csv>",
            args[0]
        );
        eprintln!("\nExample:");
        eprintln!(
            "  {} trajectory_room1.txt ./datasets/tum_vi/room1/mav0/mocap0/data.csv",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    let est_poses: Vec<(na::Vector3<f64>, na::UnitQuaternion<f64>)> = est_aligned
        .iter()
        .map(|p| (p.position, p.orientation))
        .collect();
    let gt_poses: Vec<(na::Vector3<f64>, na::UnitQuaternion<f64>)> = gt_aligned
        .iter()
        .map(|p| (p.position, p.orientation))
        .collect();
