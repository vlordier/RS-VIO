...
impl PreintegratedImu {
    ...
    pub fn integrate(&mut self, gyro: na::Vector3<f64>, accel: na::Vector3<f64>, dt: f64) {
        ...
        self.J_v_bg = self.J_v_bg - self.delta_R.to_rotation_matrix().matrix() * skew_symmetric(acc) * self.J_R_bg * dt;
        ...
        self.J_p_bg = self.J_p_bg + self.J_v_bg * dt 
                      - 0.5 * self.delta_R.to_rotation_matrix().matrix() * skew_symmetric(acc) * self.J_R_bg * dt * dt;
        
        // J_{p,ba}^{k+1} = J_{p,ba}^k + J_{v,ba}^k * dt - 0.5 * R_k * dt²
        self.J_p_ba = self.J_p_ba + self.J_v_ba * dt - 0.5 * self.delta_R.to_rotation_matrix().matrix() * dt * dt;
    ...
    fn propagate_covariance(&mut self, omega: na::Vector3<f64>, acc: na::Vector3<f64>, dt: f64) {
        ...
        A.fixed_view_mut::<3, 3>(3, 0).copy_from(&(-R_k * skew_symmetric(acc) * dt));
        
        // Position-velocity coupling
        A.fixed_view_mut::<3, 3>(6, 3).copy_from(&(na::Matrix3::identity() * dt));
        
        // Position-rotation coupling
        A.fixed_view_mut::<3, 3>(6, 0).copy_from(&(-0.5 * R_k * skew_symmetric(acc) * dt * dt));
        ...
        B.fixed_view_mut::<3, 3>(6, 3).copy_from(&(-0.5 * R_k * dt * dt));
    ...
    pub fn predict(
        ...
        let p_j = p_i + v_i * self.delta_t + R_i * self.delta_p + 0.5 * gravity * self.delta_t * self.delta_t;
