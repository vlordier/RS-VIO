...
use crate::imu::preintegration::{ImuNoise, exp_map_so3};
...
impl EskfState {
    ...
    pub fn new() -> Self {
        ...
        covariance.fixed_view_mut::<3, 3>(3, 3).fill_diagonal(0.01f64.powi(2));
        
        // Initial accel bias uncertainty: 0.1 m/s²
        covariance.fixed_view_mut::<3, 3>(6, 6).fill_diagonal(0.1f64.powi(2));
    ...
    pub fn initialize_from_static(
        ...
        self.covariance.fixed_view_mut::<3, 3>(0, 0).fill_diagonal(0.01); // Small velocity uncertainty
        self.covariance.fixed_view_mut::<3, 3>(3, 3).fill_diagonal(gyro_std.powi(2));
        self.covariance.fixed_view_mut::<3, 3>(6, 6).fill_diagonal(accel_std.powi(2));
    }
}
...
impl Eskf {
    ...
    fn predict_covariance(&mut self, dt: f64) {
        ...
        Q.fixed_view_mut::<3, 3>(3, 3).fill_diagonal(self.noise.gyro_bias_random_walk.powi(2) * dt);
        
        // Accel bias random walk
        Q.fixed_view_mut::<3, 3>(6, 6).fill_diagonal(self.noise.accel_bias_random_walk.powi(2) * dt);
    ...
    pub fn update_velocity(&mut self, measured_velocity: na::Vector3<f64>, measurement_cov: na::Matrix3<f64>) {
        // Innovation
        ...
        self.state.covariance = I_KH * self.state.covariance * I_KH.transpose() 
    ...
    pub fn update_zero_velocity(&mut self, measurement_uncertainty: f64) {
        let measurement_cov = na::SMatrix::<f64, 3, 3>::from_element(measurement_uncertainty.powi(2));
    ...
    pub fn get_bias_std(&self) -> (na::Vector3<f64>, na::Vector3<f64>) {
        ...
        let accel_std = na::Vector3::new(accel_var[0].sqrt(), accel_var[1].sqrt(), accel_var[2].sqrt());
...
mod tests {
    ...
    fn test_eskf_static_predict() {
        ...
        assert!(eskf.state.velocity.norm() < 0.1, "Velocity: {}", eskf.state.velocity.norm());
    }
}
