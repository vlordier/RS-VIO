...
mod bias_feedback_tests {
    ...
    fn test_bias_feedback_improves_estimates() {
        ...
        println!(
            "Updated biases - gyro: {:?}, accel: {:?}",
            updated_bg, updated_ba
        );

        assert!(
            (updated_bg - refined_bg).norm() < 1e-10,
            "Gyro bias should be updated"
        );
        assert!(
            (updated_ba - refined_ba).norm() < 1e-10,
            "Accel bias should be updated"
        );
        ...
        assert!(
            velocity_after.norm() < velocity_before.norm() * 1.5,
    ...
    fn test_complete_feedback_cycle() {
        ...
        let imu_data = create_synthetic_imu_with_bias(200, 0.01, true_gyro_bias, true_accel_bias);
        ...
        assert!(
            final_bg_error < iter1_bg_error,
            "Gyro bias should improve with iterations"
        );
        assert!(
            final_ba_error < iter1_ba_error,
