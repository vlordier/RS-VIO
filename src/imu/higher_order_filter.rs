...
mod tests {
    ...
    fn test_jerk_computation() {
        ...
        for _ in 0..10 {
            ...
            if filter.accel_history.len() > 2 {
                assert!(output.jerk_magnitude < 0.1, "Constant accel should have near-zero jerk");
            }
        }
    }
    ...
    fn test_jerk_spike_detection() {
        ...
        assert!(filter.jerk_spike_count > initial_count, "Should detect jerk spike");
    }
    ...
    fn test_higher_order_filter_stability() {
        ...
        for i in 0..1000 {
            ...
            assert!(output.f0_confidence.is_finite(), "f0_confidence should be finite");
        }
        
        // Should have reasonable peak magnitudes
        assert!(filter.jerk_magnitude_peak < 1000.0, "Jerk peak should be reasonable");
        assert!(filter.snap_magnitude_peak < 10000.0, "Snap peak should be reasonable");
    }
    ...
    fn test_jerk_magnitude_tracking() {
        ...
        assert!(filter.jerk_magnitude_peak > initial_peak, "Peak jerk should increase with change");
    }
    ...
    fn test_snap_magnitude_tracking() {
        ...
        assert!(filter.snap_magnitude_peak > 0.0, "Should detect snap from ramp");
    }
    ...
    fn test_statistics_accumulation() {
        ...
        assert!(stats2.peak_magnitude > stats1.peak_magnitude, "Peak should increase");
    }
    ...
    fn test_smoothing_effect() {
        ...
        let jerk_no_smooth = filter_no_smooth.process_accel([0.0, 0.0, 0.0]).jerk_magnitude;
        
        // Smoothing should reduce jerk magnitude
        assert!(jerk_with_smoothing <= jerk_no_smooth + 0.1, 
    ...
    fn test_frequency_specific_response() {
        ...
        assert!(off_confidence >= 0.0, "off-frequency confidence should be valid");
    }
    ...
    fn test_realistic_quadrotor_motion() {
        ...
        for (phase, samples, base_accel) in scenarios.iter() {
            for i in 0..*samples {
                ...
                let output = filter.process_accel(accel);
                assert!(output.jerk_magnitude.is_finite(), 
                        "Should remain stable in phase: {}", phase);
            }
        }
        ...
        assert!(jerk_stats.peak_magnitude > 0.0, "Should have detected jerk transitions");
        assert!(snap_stats.peak_magnitude > 0.0, "Should have detected snap transitions");
    }
    ...
    fn test_reset_statistics() {
        ...
        assert!(peak_before > 0.0, "Should have non-zero peak from ramp: {}", peak_before);
