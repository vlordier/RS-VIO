...
impl GFTTDetector {
    ...
    fn enforce_grid_distribution(&self, corners: Vec<(u32, u32, Float)>) -> Vec<Keypoint> {
        ...
        let mut grid: Vec<Vec<(u32, u32, Float)>> = vec![Vec::new(); (grid_w * grid_h) as usize];

        // Assign corners to grid cells
        for (x, y, score) in corners {
            let gx = (x / grid_w).min(grid_w - 1);
            let gy = (y / grid_h).min(grid_h - 1);
            let cell_idx = (gy * grid_w + gx) as usize;
            if grid[cell_idx].len() < max_per_cell as usize {
                grid[cell_idx].push((x, y, score));
            }
        }
