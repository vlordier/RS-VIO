...
impl Estimator {
    ...
    pub fn map_points_len(&self) -> usize {
        self.backend.sliding_window.map_points_len()
    }
    ...
    pub fn get_imu_preintegration(&self) -> Option<&crate::imu::PreintegratedImu> {
        self.imu_processor.current_preintegration.as_ref()
    }

    /// Get current velocity estimate
    pub fn get_velocity(&self) -> Option<Vector3> {
        if self.imu_processor.velocity_estimator.is_initialized() {
            let v = self.imu_processor.velocity_estimator.get_velocity();
    ...
    pub fn get_extrinsic_calibration(&self) -> Matrix4x4 {
        self.imu_processor.extrinsic_calibrator.get_extrinsics()
    }

    /// Get number of IMU measurements processed
    pub fn imu_measurement_count(&self) -> usize {
        self.imu_processor.stats.total_measurements
    }
    ...
    pub fn get_imu_motion_prior(&self) -> Option<ImuMotionPrior> {
        ...
        if !self.imu_processor.preintegrator.is_valid() {
        ...
        let velocity = if self.imu_processor.velocity_estimator_initialized {
            self.imu_processor.velocity_estimator.get_velocity()
        } else {
            self.imu_processor.current_velocity
        };
    ...
    pub fn reset_keyframe_selector(&mut self) {
        self.imu_processor.keyframe_selector.reset();
    }

    /// Get IMU measurement rate (for monitoring)
    pub fn get_imu_rate(&self) -> f64 {
        if self.imu_processor.stats.total_measurements > 0 && self.frame_id_counter > 0 {
            self.imu_processor.stats.total_measurements as f64 / self.frame_id_counter as f64
        } else {
            0.0
        }
    }
