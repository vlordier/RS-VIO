...
impl RelativePoseError {
    ...
    pub fn calculate(
        ...
        let trans_rmse =
            (trans_errors.iter().map(|e| e * e).sum::<f64>() / trans_errors.len() as f64).sqrt();

        let rot_mean = rot_errors.iter().sum::<f64>() / rot_errors.len() as f64;
        let rot_rmse =
            (rot_errors.iter().map(|e| e * e).sum::<f64>() / rot_errors.len() as f64).sqrt();
...
mod tests {
    ...
    fn test_rpe_with_drift() {
        let estimated = vec![
            ...
            (
                na::Vector3::new(1.0, 0.05, 0.0),
                na::UnitQuaternion::identity(),
            ),
            (
                na::Vector3::new(2.0, 0.10, 0.0),
                na::UnitQuaternion::identity(),
            ),
        ];
