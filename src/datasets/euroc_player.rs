...
impl EurocPlayer {
    ...
    pub fn run(&self, config: PlayerConfig) -> PlayerResult {
        ...
        let imu_data = match Self::load_imu_data(&config.dataset_path) {
            Ok(data) => {
                if !data.is_empty() {
                    log::info!("[EurocPlayer] Loaded {} IMU samples", data.len());
                }
                data
            },
            Err(e) => {
                log::warn!("[EurocPlayer] Failed to load IMU data: {}", e);
                Vec::new()
            },
        };
    ...
    fn load_imu_data(dataset_path: &str) -> Result<Vec<ImuData>> {
        ...
        if !imu_file.exists() {
            log::debug!("[EurocPlayer] IMU file not found at {:?}, skipping", imu_file);
            return Ok(Vec::new());
        }
        ...
        log::info!("[EurocPlayer] Loaded {} IMU samples from {}", imu_data.len(), imu_file.display());
        Ok(imu_data)
    }
    ...
    fn save_statistics(result: &PlayerResult, dataset_path: &str) {
        ...
        if let Ok(mut file) = std::fs::File::create(&stats_file) {
            ...
            log::info!(
                "[EurocPlayer] Saved statistics to: {}",
                stats_file.display()
            );
        }
    }
