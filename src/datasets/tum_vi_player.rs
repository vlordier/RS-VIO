...
impl DatasetPlayer for TUMVIPlayer {
    ...
    fn load_imu_data(
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            let timestamp: i64 = ok_or_log!(
                parts[0].trim().parse(),
                0,
                "Failed to parse TUM-VI IMU timestamp on line {}",
                line_num + 1
            );
            ...
            let gyro_x: f64 = ok_or_log!(
                parts[1].trim().parse(),
                0.0,
                "Failed to parse TUM-VI gyro_x on line {}",
                line_num + 1
            );
            let gyro_y: f64 = ok_or_log!(
                parts[2].trim().parse(),
                0.0,
                "Failed to parse TUM-VI gyro_y on line {}",
                line_num + 1
            );
            let gyro_z: f64 = ok_or_log!(
                parts[3].trim().parse(),
                0.0,
                "Failed to parse TUM-VI gyro_z on line {}",
                line_num + 1
            );
            ...
            let accel_x: f64 = ok_or_log!(
                parts[4].trim().parse(),
                0.0,
                "Failed to parse TUM-VI accel_x on line {}",
                line_num + 1
            );
            let accel_y: f64 = ok_or_log!(
                parts[5].trim().parse(),
                0.0,
                "Failed to parse TUM-VI accel_y on line {}",
                line_num + 1
            );
            let accel_z: f64 = ok_or_log!(
                parts[6].trim().parse(),
                0.0,
                "Failed to parse TUM-VI accel_z on line {}",
                line_num + 1
            );
    ...
    fn get_imu_data_between_frames(
        ...
        self.imu_cache
            ...
            .unwrap_or_else(|e| {
                log::warn!("TUM-VI IMU cache poisoned while fetching between frames: {}", e);
                Vec::new()
            })
    }
