...
impl TeacherLabelExporterJson {
    ...
    pub fn export_frame(&mut self, frame: super::TeacherFrame) -> Result<()> {
        ...
        let depth_coverage = frame.depth_map
            .iter()
            .filter(|&&d| d > 0.0)
            .count() as f32
        ...
        let flow_vec: Vec<f64> = frame.flow_grid
            .iter()
            .flat_map(|point| {
    ...
    pub fn finalize(&mut self) -> Result<()> {
        ...
        let metadata_path = self.output_dir.join(format!("{}_metadata.json", self.sequence_name));
        ...
        let summary = ExportSummary {
            ...
            average_depth_coverage: self.frames_metadata
                .iter()
                .map(|f| f.depth_coverage as f64)
                .sum::<f64>() / self.frame_count.max(1) as f64,
            average_reprojection_error: self.frames_metadata
                .iter()
                .map(|f| f.mean_reprojection_error as f64)
                .sum::<f64>() / self.frame_count.max(1) as f64,
        };
        
        let summary_path = self.output_dir.join(format!("{}_summary.json", self.sequence_name));
