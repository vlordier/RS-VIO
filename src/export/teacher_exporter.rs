...
impl TeacherLabelExporter {
    ...
    pub fn export_frame(&mut self, frame: TeacherFrame) -> Result<()> {
        ...
        log::debug!("Exporting teacher frame {}: id={}, timestamp={:.3}s", 
                   idx, frame.frame_id, frame.timestamp);
        ...
        // Write IMU
        self.write_vector(&format!("imu/preintegration_{}", idx), frame.imu_preintegration.as_slice())?;
        self.write_vector(&format!("imu/covariance_{}", idx), frame.imu_covariance.as_slice())?;
        self.write_scalar(&format!("imu/sample_count_{}", idx), frame.imu_sample_count as f64)?;
        ...
        // Write tracking data
        self.write_vector(&format!("tracking/match_quality_{}", idx), &frame.match_quality)?;
        self.write_scalar(&format!("tracking/time_since_keyframe_{}", idx), frame.time_since_keyframe)?;
        ...
        self.write_vector(&format!("history/previous_velocity_{}", idx), frame.previous_velocity.as_slice())?;
        
        // Write labels (ground truth)
        self.write_se3(&format!("labels/pose_{}", idx), &frame.pose_world_cam)?;
        self.write_matrix6(&format!("labels/covariance_{}", idx), &frame.pose_covariance)?;
        self.write_vector(&format!("labels/velocity_{}", idx), frame.velocity.as_slice())?;
        self.write_array2(&format!("labels/depth_{}", idx), &frame.depth_map)?;
        self.write_array2(&format!("labels/depth_confidence_{}", idx), &frame.depth_confidence)?;
        
        // Write quality metrics
        self.write_vector_f32(&format!("quality/reproj_errors_{}", idx), &frame.reprojection_errors)?;
        self.write_scalar(&format!("quality/mean_reproj_error_{}", idx), frame.mean_reprojection_error as f64)?;
        self.write_scalar(&format!("quality/ba_iterations_{}", idx), frame.ba_iterations as f64)?;
        self.write_scalar(&format!("quality/ba_converged_{}", idx), if frame.ba_converged { 1.0 } else { 0.0 })?;
        ...
        if self.frame_count % 50 == 0 {
            log::info!("Exported {} teacher frames for sequence '{}'", 
                      self.frame_count, self.sequence_name);
        }
    ...
    pub fn finalize(&mut self) -> Result<()> {
        log::info!("Finalizing teacher data export: {} frames for sequence '{}'", 
                  self.frame_count, self.sequence_name);
        
        // Write summary metadata
        let metadata = self.file.group("metadata")?;
        metadata.new_dataset::<u64>()
            .create("total_frames")?
            .write_scalar(&(self.frame_count as u64))?;
        
        self.file.flush()?;
        
        log::info!("✅ Teacher data export complete: {} frames", self.frame_count);
