...
impl DatasetExporter {
    ...
    fn load_imu_data(dataset_path: &PathBuf) -> Result<Vec<ImuMeasurement>> {
        ...
        for line in reader.lines() {
            ...
            let parts: Vec<&str> = line.split(',').collect();
            if parts.len() >= 7 {
                if let (Ok(timestamp_ns), Ok(gyr_x), Ok(gyr_y), Ok(gyr_z), Ok(acc_x), Ok(acc_y), Ok(acc_z)) = (
                    parts[0].parse::<u64>(),
                    parts[1].parse::<f32>(),
                    parts[2].parse::<f32>(),
                    parts[3].parse::<f32>(),
    ...
    fn process_real_dataset(&mut self, max_frames: usize) -> Result<()> {
        ...
        for (idx, camera_frame) in self.camera_frames.iter().take(max_frames).enumerate() {
            ...
            if (idx + 1) % 100 == 0 {
                ...
                println!("   Processed {}/{} frames... ({:.1} fps)", 
                         idx + 1, max_frames, fps);
            }
        }
    ...
    fn write_metadata(&self) -> Result<()> {
        ...
        println!("   File size: {:.2} MB", fs::metadata(&metadata_path)?.len() as f64 / 1_000_000.0);
