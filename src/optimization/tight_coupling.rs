...
impl BiasRefinement {
    ...
    pub fn is_valid(&self) -> bool {
        self.accel_bias.norm() < self.max_accel_bias &&
        self.gyro_bias.norm() < self.max_gyro_bias
    }
    ...
    pub fn update_from_residuals(
        ...
        self.accel_bias.x = self.accel_bias.x.clamp(-self.max_accel_bias, self.max_accel_bias);
        self.accel_bias.y = self.accel_bias.y.clamp(-self.max_accel_bias, self.max_accel_bias);
        self.accel_bias.z = self.accel_bias.z.clamp(-self.max_accel_bias, self.max_accel_bias);
        
        self.gyro_bias.x = self.gyro_bias.x.clamp(-self.max_gyro_bias, self.max_gyro_bias);
        self.gyro_bias.y = self.gyro_bias.y.clamp(-self.max_gyro_bias, self.max_gyro_bias);
        self.gyro_bias.z = self.gyro_bias.z.clamp(-self.max_gyro_bias, self.max_gyro_bias);
    }
}
