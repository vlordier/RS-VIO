...
impl CalibrationConfidenceFactors {
    ...
    fn extract_metric_score(report: &CalibrationQualityReport, metric_prefix: &str) -> f32 {
        ...
        let matching_metrics: Vec<bool> = report.metrics
            .iter()
...
impl CalibrationQualityStats {
    ...
    pub fn compute_improvement(&self) -> CalibrationImprovementMetrics {
        ...
        CalibrationImprovementMetrics {
            visual_rms_improvement: (unweighted_visual_rms - weighted_visual_rms) / unweighted_visual_rms,
...
mod tests {
    ...
    fn test_perfect_calibration_confidence() {
        let report = CalibrationQualityReport {
            passed: true,
            metrics: vec![
                ("reprojection_rms".to_string(), true, "Excellent".to_string()),
                ("vertical_disparity".to_string(), true, "Good".to_string()),
                ("timing_sharpness".to_string(), true, "Observable".to_string()),
                ("imu_integration".to_string(), true, "Consistent".to_string()),
                ("rolling_shutter_significance".to_string(), true, "Handled".to_string()),
            ],
            overall_score: 0.98,
        };
    ...
    fn test_poor_calibration_confidence() {
        let report = CalibrationQualityReport {
            ...
            metrics: vec![
                ...
                ("timing_sharpness".to_string(), false, "Unobservable".to_string()),
                ("imu_integration".to_string(), false, "Inconsistent".to_string()),
                ("rolling_shutter_significance".to_string(), false, "Problematic".to_string()),
            ],
            overall_score: 0.25,
        };
    ...
    fn test_marginal_calibration() {
        let report = CalibrationQualityReport {
            passed: true,
            metrics: vec![
                ("reprojection_rms".to_string(), true, "Acceptable".to_string()),
                ("vertical_disparity".to_string(), false, "Marginal".to_string()),
    ...
    fn test_rolling_shutter_gating() {
        let report_good_rs = CalibrationQualityReport {
            passed: true,
            metrics: vec![
                ("rolling_shutter_significance".to_string(), true, "Good".to_string()),
                ("rolling_shutter_readout".to_string(), true, "Accurate".to_string()),
            ],
            overall_score: 0.95,
        };
        ...
        let report_bad_rs = CalibrationQualityReport {
            passed: false,
            metrics: vec![
                ("rolling_shutter_significance".to_string(), false, "Poor".to_string()),
            ],
            overall_score: 0.4,
        };
    ...
    fn test_robust_threshold_adaptation() {
        ...
        assert!(factors_poor.robust_threshold_multiplier() > factors_perfect.robust_threshold_multiplier());
        assert!((factors_perfect.robust_threshold_multiplier() - 1.0).abs() < 0.01); // ~1.0 for perfect
    }
