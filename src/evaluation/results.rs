...
use crate::evaluation::{DepthMetrics, FeatureMetrics, RobustnessMetrics, TrajectoryMetrics};
...
impl ConfigurationResults {
    ...
    pub fn to_string(&self) -> String {
        ...
        if let Some(ref robust) = self.robustness {
            ...
            output.push_str(&format!(
                "  Avg Failure Duration: {:.1} frames\n",
                robust.avg_failure_duration
            ));
            output.push_str(&format!(
                "  Max Failure Duration: {} frames\n",
...
impl ComparisonResults {
    ...
    pub fn summary(&self) -> String {
        ...
        if let Some(baseline) = baseline {
            ...
            for config in &self.configurations {
                let overhead =
                    ((config.computation_time_ms / baseline_time - 1.0) * 100.0).max(0.0);
                output.push_str(&format!(
                    "{:<28} | {:.3} ms | {:.1}%\n",
                    config.config.name(),
