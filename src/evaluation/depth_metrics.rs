...
pub fn compute_depth_rmse(
    ...
    // Compute statistics
    let rmse = (depth_errors.iter()
        .map(|&e| e * e)
        .sum::<f64>() / depth_errors.len() as f64).sqrt();
    
    let mae = depth_errors.iter().sum::<f64>() / depth_errors.len() as f64;
    
    let mean_error = mae;
    let std_dev = (depth_errors.iter()
        .map(|&e| (e - mean_error).powi(2))
        .sum::<f64>() / depth_errors.len() as f64).sqrt();
    
    let outliers = depth_errors.iter()
        .filter(|&&e| e > 2.0 * std_dev)
        .count();
    let outlier_pct = (outliers as f64 / depth_errors.len() as f64) * 100.0;
    
    let disparity_rmse = (disparity_errors.iter()
        .map(|&e| e * e)
        .sum::<f64>() / disparity_errors.len() as f64).sqrt();
