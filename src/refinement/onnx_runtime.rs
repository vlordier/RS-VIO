...
impl RefinementNetwork {
    ...
    pub fn new(model_path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        log::info!(
            "[RefinementNetwork] Input names: {:?}",
            session.inputs.iter().map(|i| &i.name).collect::<Vec<_>>()
        );
        log::info!(
            "[RefinementNetwork] Output names: {:?}",
    ...
    pub fn refine(
        ...
        let correction_slice = correction_tensor
            .as_slice()
            .ok_or("Failed to extract correction")?;

        if correction_slice.len() != 3 {
            return Err(format!(
                "Invalid correction size: expected 3, got {}",
