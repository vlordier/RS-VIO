...
impl OptimizedFusionAlgorithm {
    ...
    pub fn with_calibration(config: AdaptiveFusionConfig, calibration: CalibrationResult) -> Self {
        ...
        let calibration_confidence = CalibrationConfidenceFactors::from_quality_report(
            &quality_report
        );
    ...
    pub fn update_calibration(&mut self, calibration: CalibrationResult) {
        ...
        let thresholds = AcceptanceThresholds::standard();
        let quality_report = calibration.quality_report(&thresholds);
        self.calibration_confidence = CalibrationConfidenceFactors::from_quality_report(
            &quality_report
        );
    ...
    pub fn adaptive_confidence(
        ...
        let combined = (denoise_confidence * super_res_confidence
            * distance_factor
            * speed_factor
            * calibration_factor)
            .clamp(0.1, 1.0);
    ...
    pub fn predict_pixel_with_imu(
        ...
        let u_pred = camera_intrinsics_fx * (ray_camera_rotated.x / ray_camera_rotated.z) + camera_intrinsics_cx;
        let v_pred = camera_intrinsics_fy * (ray_camera_rotated.y / ray_camera_rotated.z) + camera_intrinsics_cy;
    ...
    pub fn rolling_shutter_time_offset(&self, row: usize, image_height: usize, camera_id: &str) -> Option<f64> {
        let calibration = self.calibration.as_ref()?;
    ...
    pub fn estimate_3d_resolution(
        ...
        let base_improvement = baseline_uncertainty
            * denoise_factor as f64
            * super_res_factor as f64;
...
mod tests {
    ...
    use crate::calibration::types::{
        CalibrationResult, CameraIMUCalibrationResult, CameraIMUExtrinsics,
        RollingShutterDetectionResult, TimingQuality, IMUCalibrationResult,
        IMUIntrinsics, CameraCalibrationResult, CameraIntrinsics,
    };
    use std::collections::HashMap;
    
    fn create_test_calibration(overall_score: f64, rs_significant: bool) -> CalibrationResult {
        let mut cameras = HashMap::new();
        cameras.insert("cam0".to_string(), CameraCalibrationResult {
        ...
        camera_imu_extrinsics.insert("cam0".to_string(), CameraIMUCalibrationResult {
        ...
        rolling_shutter.insert("cam0".to_string(), RollingShutterDetectionResult {
        ...
        CalibrationResult {
            ...
            rolling_shutter,
            timing_quality: TimingQuality::Stable { offset_s: 0.001, jitter_s: 1e-5 },
    ...
    fn test_adaptive_confidence_with_calibration() {
        ...
        let mut algo_excellent = OptimizedFusionAlgorithm::with_calibration(
            config.clone(),
            excellent_calibration,
        );
        
        // Poor calibration
        let poor_calibration = create_test_calibration(0.30, false);
        let mut algo_poor = OptimizedFusionAlgorithm::with_calibration(
            config,
            poor_calibration,
        );
    ...
    fn test_rolling_shutter_time_offset() {
        ...
        let t_top = algo.rolling_shutter_time_offset(0, image_height, "cam0").unwrap();
        assert!((t_top - 0.0).abs() < 1e-6);
        
        // Middle row: half readout
        let t_mid = algo.rolling_shutter_time_offset(image_height / 2, image_height, "cam0").unwrap();
        assert!((t_mid - 0.0165).abs() < 1e-3); // ~16.5ms
        
        // Bottom row: full readout
        let t_bot = algo.rolling_shutter_time_offset(image_height - 1, image_height, "cam0").unwrap();
