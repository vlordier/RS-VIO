...
impl MotionAwareDepthOptimizer {
    ...
    pub fn optimize_depth(
        ...
        // Combined confidence
        let confidence =
            (base_confidence * distance_confidence_boost * speed_confidence).clamp(0.1, 1.0);
    ...
    pub fn detect_anomaly(&self, new_depth: &MotionConstrainedDepth) -> bool {
        ...
        let recent: Vec<_> = self.depth_history.iter().rev().take(3).collect();
...
mod tests {
    ...
    fn test_depth_optimization() {
        ...
        let depth = optimizer.optimize_depth(position, 0.05, &constraints, 0.54);
