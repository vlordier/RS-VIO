...
use rerun::{components::Color, RecordingStream};
...
pub fn log_points_2d(
    ...
    let rerun_points: Vec<_> = points.iter().map(|p| [p[0] as f32, p[1] as f32]).collect();
...
pub fn log_text(rec: &RecordingStream, entity_path: &str, text: String) {
    rec.log(entity_path, &rerun::TextDocument::new(text)).ok();
}
