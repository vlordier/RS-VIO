...
def benchmark_pipeline():
    ...
    if gpu_justified:
        ...
        print("  Expected speedup: ~4-8x (GPU vs CPU)")
        print(f"  Potential savings: {total_avg * 0.85:.2f}-{total_avg * 0.875:.2f} ms per frame")
    else:
        print("✗ GPU ACCELERATION NOT JUSTIFIED")
        print(f"  {total_avg:.4f} ms ({gpu_percentage:.1f}%) < {GPU_DECISION_THRESHOLD*100:.0f}% threshold")
        print("  Feature tracking is already fast enough on CPU")
        print("  GPU overhead would outweigh benefits")
