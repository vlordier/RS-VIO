...
fn create_test_image(width: u32, height: u32) -> GrayImage {
    ...
    for y in 0..height {
        for x in 0..width {
            let value = u8::try_from((x + y) % 256).unwrap_or(0);
    ...
    for (width, height) in &[(320, 240), (640, 480), (752, 480)] {
        ...
        for buffer_size in &[3, 5, 7] {
            ...
            group.bench_with_input(
                ...
                buffer_size,
                |b, _| {
                    b.iter(|| black_box(stabilizer.process_frame(&image, rotation, timestamp)));
                },
            );
        }
    }
...
fn bench_track_first_detector(c: &mut Criterion) {
    ...
    for i in 0..150 {
        ...
        tracked_points.push((i as usize, na::Vector2::new(x, y)));
    }
    ...
    for max_features in &[150, 250, 350] {
        ...
        group.bench_with_input(
            ...
            max_features,
            |b, _| {
                b.iter(|| black_box(detector.update_tracks(&image, &tracked_points, None)));
            },
        );
    }
...
fn bench_track_first_needs_detection(c: &mut Criterion) {
    ...
    for num_features in &[50, 100, 150, 200, 250] {
        ...
        group.bench_with_input(
            ...
            num_features,
            |b, _| {
                b.iter(|| black_box(detector.needs_new_features()));
            },
        );
    }
