...
use rs_vio::evaluation::{
    compute_ate,
    compute_depth_rmse, compute_reprojection_error, track_failure_rate,
};
...
fn add_error(
    ...
    poses.iter().enumerate().map(|(i, pose)| {
...
fn bench_depth_accuracy(c: &mut Criterion) {
    ...
    let baseline_points: Vec<_> = ground_truth_points.iter().enumerate().map(|(i, &pt)| {
        [
    ...
    let imu_points: Vec<_> = ground_truth_points.iter().enumerate().map(|(i, &pt)| {
        [
    ...
    let super_res_points: Vec<_> = ground_truth_points.iter().enumerate().map(|(i, &pt)| {
        [
    ...
    let fusion_points: Vec<_> = ground_truth_points.iter().enumerate().map(|(i, &pt)| {
        [
...
fn bench_feature_quality(c: &mut Criterion) {
    ...
    let baseline_2d: Vec<_> = actual_2d.iter().enumerate().map(|(i, &(x, y))| {
        (
    ...
    group.bench_function("baseline_reprojection", |b| {
        b.iter(|| {
            let _ = compute_reprojection_error(
                black_box(&baseline_2d),
                black_box(&actual_2d),
                None,
            );
        });
    ...
    // IMU only: Moderate improvement
    let imu_2d: Vec<_> = actual_2d.iter().enumerate().map(|(i, &(x, y))| {
        (
    ...
    let super_res_2d: Vec<_> = actual_2d.iter().enumerate().map(|(i, &(x, y))| {
        (
    ...
    let fusion_2d: Vec<_> = actual_2d.iter().enumerate().map(|(i, &(x, y))| {
        (
...
fn bench_robustness(c: &mut Criterion) {
    ...
    group.bench_function("imu_only_robustness", |b| {
        b.iter(|| {
            let _ = track_failure_rate(
                black_box(&imu_errors),
                black_box(&imu_confidence),
                0.4,
            );
        });
    });
