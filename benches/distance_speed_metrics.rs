...
fn generate_test_trajectory(
    ...
    for i in 0..num_frames {
        ...
        let pos = match trajectory_type {
            ...
            "zigzag" => {
                let amp = ((t / 2.0).floor() as i32 % 2 == 0).then_some(1.0).unwrap_or(-1.0);
        ...
        let velocity = Vector3::new(
            pos.x - trajectory.last().map(|(p, _, _): &(Point3<f64>, _, _)| p.x).unwrap_or(0.0),
            pos.y - trajectory.last().map(|(p, _, _): &(Point3<f64>, _, _)| p.y).unwrap_or(0.0),
            pos.z - trajectory.last().map(|(p, _, _): &(Point3<f64>, _, _)| p.z).unwrap_or(5.0),
        ) / dt;
...
pub fn benchmark_speed_performance(c: &mut Criterion) {
    ...
    for (speed_name, speed) in &speeds {
        group.bench_function(
            format!("baseline_speed_{}", speed_name),
            |b| {
                let _trajectory = black_box(generate_test_trajectory(
                    10.0,
                    300,
                    "spiral",
                ));
                ...
                let scaled_traj: Vec<_> = vec![
                    (Point3::new(0.0, 0.0, 5.0), Vector3::new(speed / 1.0, 0.0, 0.0), Vector3::<f64>::zeros());
                    100
                ];
    ...
    for (speed_name, speed) in &speeds {
        group.bench_function(
            format!("fusion_speed_{}", speed_name),
            |b| {
                let _trajectory = black_box(generate_test_trajectory(
                    10.0,
                    300,
...
pub fn benchmark_distance_speed_combined(c: &mut Criterion) {
    ...
    for (scenario_name, distance, speed) in &scenarios {
        group.bench_function(
            format!("baseline_{}", scenario_name),
            |b| {
